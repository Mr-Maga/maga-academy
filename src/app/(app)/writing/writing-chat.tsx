...
export function WritingChat({
  ...
  async function send() {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/ai/writing-chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ question, essay, evalSummary, messages: next }),
      });
  ...
  return (
    <div className="card space-y-3 p-4">
      <h3 className="flex items-center gap-1.5 font-semibold text-primary-soft">
        <MessageCircle className="h-4 w-4" /> Maga bilan suhbat — shu yozuvingiz haqida
      </h3>
      {messages.length === 0 && (
        <p className="text-sm text-muted">
          Savol bering: “Xulosamni qanday kuchaytiraman?”, “Qaysi grammatikani tuzatay?”, “Nega shu band oldim?”…
        </p>
      )}
      {messages.length > 0 && (
